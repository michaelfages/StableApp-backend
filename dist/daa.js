...
exports.deferredAcceptanceAlgorithm = deferredAcceptanceAlgorithm;
/**
 ...
 */
function deferredAcceptanceAlgorithm(menPrefs, womenPrefs) {
    ...
    const freeMen = [];
    const proposals = Array(n).fill(0); // proposals[i]: next woman to propose for man i
    const engagedTo = Array(n).fill(-1); // woman j is engaged to man i, or -1 if free
    for (let i = 0; i < n; i++)
        freeMen.push(i);
